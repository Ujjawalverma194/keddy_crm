...
async function list(req, res) {
  const { page, pageSize, skip, limit } =
    drfPaginate(req.query);
  ...
  // 👇 YAHAN Step 5 start
  let items = await Client.find(filter).sort({
    createdAt: -1,
  });
...
return (
  Number(client.createdById) === userId ||
  assignedIds.map(Number).includes(userId)
);
  });
