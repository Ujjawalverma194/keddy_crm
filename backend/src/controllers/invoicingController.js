...
async function updateInvoice(req, res) {
  const invoiceId = parseInt(req.params.pk || req.params.id, 10);
...
async function candidateInvoiceHistory(req, res) {
  const companyId = resolveCompanyId(req.user) || req.user.id;
  const candidateId = parseInt(req.params.candidate_id, 10);
  // The generic repository cannot reliably query JSON arrays across all dialects,
  // so fetch company invoices and filter in memory to preserve Django API behaviour.
  const items = await Invoice.find({ companyId, isDeleted: false }).sort({ createdAt: -1 });

  const filtered = items.filter((i) => {
    if (i.candidateId === candidateId) return true;
    return Array.isArray(i.items) && i.items.some((item) => String(item.candidate) === String(candidateId));
  });
...
async function updateInvoicePaymentStatus(invoiceId) {
  const invoice = await Invoice.findOne({ id: invoiceId, isDeleted: false });
  if (!invoice) return null;
  const payments = await InvoicePayment.find({ invoiceId });
  const paidAmount = payments.reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
  const total = Number(invoice.totalAmount) || 0;
  if (total > 0 && paidAmount >= total) invoice.paymentStatus = 'PAID';
...
async function listPayments(req, res) {
  ...
  const payments = invoiceIds.length ? await InvoicePayment.find({ invoiceId: { $in: invoiceIds } }).sort({ createdAt: -1 }) : [];
  return res.json({
...
async function createPayment(req, res) {
  const companyId = resolveCompanyId(req.user) || req.user.id;
  const b = req.body || {};
  const invoiceId = parseInt(b.invoice || b.invoice_id, 10);
  const invoice = await Invoice.findOne({ id: invoiceId, companyId, isDeleted: false });
  if (!invoice) return res.status(404).json({ detail: 'Invoice not found or does not belong to your company.' });

  const payment = await InvoicePayment.create({
    invoiceId,
    amount: parseFloat(b.amount || 0),
    paymentDate: b.payment_date ? new Date(b.payment_date) : new Date(),
    paymentMode: b.payment_mode || b.mode || '',
    referenceNumber: b.reference_number || b.reference || '',
    notes: b.notes || '',
  ...
  return res.status(201).json({
    ...
    notes: payment.notes,
    created_by: payment.recordedById,
    created_at: payment.createdAt,
  });
}

async function financeClientDetail(req, res) {
  const companyIds = await getCompanyUserIds(req.user);
  const id = parseInt(req.params.client_id || req.params.id, 10);
  const client = await Client.findOne({
    id,
    isDeleted: false,
    createdById: { $in: companyIds },
