...
- **PostgreSQL data**: Restore production data directly into PostgreSQL using your existing dump/backup process.
...
## PostgreSQL-native migration note

This backend now uses Sequelize/PostgreSQL only. The old database compatibility files were removed from `src/utils`; model exports are backed by Sequelize models and PostgreSQL query helpers. Keep production credentials in `.env` and do not commit them.
