...
//     if (loading) return <BaseLayout><div style={styles.loader}>Loading Client Profile...</div></BaseLayout>;
//     if (!client) return <BaseLayout><div style={styles.error}>Client not found!</div></BaseLayout>;
...
//                         <h2 style={styles.pageTitle}>{client.client_name}</h2>
//                         <div style={styles.badgeGroup}>
//                             {client.is_verified ? <span style={styles.verifiedBadge}>Verified</span> : <span style={styles.unverifiedBadge}>Unverified</span>}
//                             {!client.is_active && <span style={styles.inactiveBadge}>Inactive</span>}
//                         </div>
//                     </div>
//                     <span style={styles.companySub}>{client.company_name} • Created by {client.created_by_name}</span>
//                 </div>
//             </div>
...
//             <div style={styles.statsRow}>
//                 <div style={styles.statCard}>
//                     <span style={styles.statLabel}>Total Profiles Shared</span>
//                     <span style={styles.statValue}>{filteredCount}</span>
//                 </div>
                
//                 <div style={styles.filterCard}>
//                     <div style={styles.filterTitle}>Filter Profiles by Date</div>
//                     <div style={styles.dateInputs}>
//                         <div style={styles.inputGroup}>
//                             <label style={styles.inputLabel}>Start Date</label>
//                             <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} style={styles.dateInput} />
...
//                             <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} style={styles.dateInput} />
//                         </div>
//                         <button onClick={handleFilter} style={styles.filterBtn} disabled={filterLoading}>
//                             {filterLoading ? "..." : "Apply Filter"}
//                         </button>
//                         {(startDate || endDate) && (
//                             <button onClick={() => {setStartDate(""); setEndDate(""); fetchClientDetails();}} style={styles.resetBtn}>Reset</button>
//                         )}
//                     </div>
//                 </div>
//             </div>
...
//                     <div style={styles.card}>
//                         <h3 style={styles.cardHeading}>Company Details</h3>
//                         <div style={styles.infoGrid}>
//                             <div style={styles.fullRow}>
//                                 <InfoItem label="Company Name" value={client.company_name} />
//                             </div>
...
//                         <DocFileRow 
//                             label="NDA Document" 
//                             fileUrl={client.nda_document} 
//                             status={client.nda_status}
//                             date={client.nda_uploaded_date}
//                             styleFunc={getStatusStyle}
//                         />

//                         <DocFileRow 
//                             label="MSA Document" 
//                             fileUrl={client.msa_document} 
//                             status={client.msa_status}
//                             date={client.msa_uploaded_date}
//                             styleFunc={getStatusStyle}
//                         />
//                     </div>
...
//                     <div style={styles.card}>
//                         <h3 style={styles.cardHeading}>Internal Remark</h3>
//                         <p style={styles.remarkText}>{client.remark || "No remarks added for this client."}</p>
//                     </div>

//                     <div style={styles.card}>
//                         <h3 style={styles.cardHeading}>System Metadata</h3>
//                         <div style={styles.metaInfo}>
//                             <p><strong>Added By:</strong> {client.created_by_name}</p>
//                             <p><strong>Created On:</strong> {new Date(client.created_at).toLocaleString()}</p>
//                             <p><strong>Assigned Emp:</strong> {client.assigned_employees.length > 0 ? client.assigned_employees.join(', ') : "None"}</p>
//                         </div>
//                     </div>
//                 </div>
//             </div>
...
// const DocFileRow = ({ label, fileUrl, status, date, styleFunc }) => (
//     <div style={styles.docRow}>
//         <div style={{ flex: 1 }}>
//             <div style={styles.docLabel}>{label}</div>
//             <div style={styles.docDate}>{date ? `Uploaded: ${date}` : "Not uploaded"}</div>
//         </div>
//         <div style={styles.docActionGroup}>
//             <span style={{ ...styles.statusBadge, ...styleFunc(status) }}>{status.replace('_', ' ')}</span>
//             {fileUrl && <a href={fileUrl} target="_blank" rel="noreferrer" style={styles.fileLink}>Open</a>}
//         </div>
//     </div>
// );
...
//     pageTitle: { fontSize: "24px", color: "#1E293B", fontWeight: "800", margin: 0 },
//     badgeGroup: { display: "flex", gap: "8px" },
//     companySub: { fontSize: "14px", color: "#64748B", fontWeight: "500" },
    
//     statsRow: { display: "flex", gap: "20px", marginBottom: "25px", flexWrap: "wrap" },
//     statCard: { background: "#3B82F6", padding: "20px", borderRadius: "16px", flex: "1", minWidth: "200px", color: "#fff", textAlign: "center" },
//     statLabel: { fontSize: "13px", fontWeight: "600", opacity: 0.9, textTransform: "uppercase" },
//     statValue: { fontSize: "36px", fontWeight: "800", display: "block" },
    
//     filterCard: { background: "#fff", padding: "20px", borderRadius: "16px", flex: "2.5", minWidth: "320px", border: "1px solid #E2E8F0" },
//     filterTitle: { fontSize: "11px", fontWeight: "800", color: "#94A3B8", marginBottom: "12px", textTransform: "uppercase" },
//     dateInputs: { display: "flex", gap: "12px", alignItems: "flex-end", flexWrap: "wrap" },
//     inputGroup: { display: "flex", flexDirection: "column", gap: "5px", flex: 1 },
//     inputLabel: { fontSize: "11px", fontWeight: "700", color: "#64748B" },
//     dateInput: { padding: "8px 12px", borderRadius: "8px", border: "1px solid #E2E8F0", outline: "none", fontSize: "14px" },
//     filterBtn: { background: "#1E293B", color: "#fff", border: "none", padding: "10px 20px", borderRadius: "8px", fontWeight: "700", cursor: "pointer" },
//     resetBtn: { background: "transparent", color: "#64748B", border: "1px solid #E2E8F0", padding: "10px 15px", borderRadius: "8px", cursor: "pointer" },

//     containerGrid: { display: "grid", gridTemplateColumns: "2fr 1fr", gap: "20px" },
//     leftCol: { display: "flex", flexDirection: "column", gap: "20px" },
//     rightCol: { display: "flex", flexDirection: "column", gap: "20px" },

//     card: { background: "#fff", padding: "20px", borderRadius: "16px", border: "1px solid #E2E8F0" },
//     cardHeading: { fontSize: "12px", fontWeight: "800", color: "#94A3B8", marginBottom: "15px", textTransform: "uppercase", borderBottom: "1px solid #F8FAFC", paddingBottom: "8px" },
...
//     docRow: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #F1F5F9" },
//     docLabel: { fontSize: "14px", fontWeight: "700", color: "#1E293B" },
//     docDate: { fontSize: "11px", color: "#94A3B8" },
//     docActionGroup: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px" },
//     statusBadge: { padding: "2px 8px", borderRadius: "4px", fontSize: "10px", fontWeight: "800" },
//     fileLink: { fontSize: "11px", color: "#3B82F6", textDecoration: "none", fontWeight: "700", background: "#EFF6FF", padding: "4px 8px", borderRadius: "6px" },

//     verifiedBadge: { background: "#DCFCE7", color: "#166534", padding: "2px 8px", borderRadius: "4px", fontSize: "11px", fontWeight: "800" },
//     unverifiedBadge: { background: "#FEF3C7", color: "#92400E", padding: "2px 8px", borderRadius: "4px", fontSize: "11px", fontWeight: "800" },
//     inactiveBadge: { background: "#FEE2E2", color: "#991B1B", padding: "2px 8px", borderRadius: "4px", fontSize: "11px", fontWeight: "800" },
//     remarkText: { fontSize: "14px", color: "#475569", fontStyle: "italic", background: "#FFFBEB", padding: "12px", borderRadius: "8px", borderLeft: "4px solid #F59E0B" },
//     metaInfo: { fontSize: "13px", color: "#64748B", display: "flex", flexDirection: "column", gap: "8px" }
// };
