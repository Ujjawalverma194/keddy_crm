...
const getCurrentUserId = () => {
  try {
    ...
    return (
      payload.user_id || payload.userId || payload.id || payload.pk || null
    );
  } catch (error) {
    return null;
  }
...
function EmployeeDashboard() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const hasClientSubmission = (candidate) =>
        Boolean(
          candidate?.client ||
          candidate?.client_id ||
          candidate?.clientId ||
      ...
      const getCount = (d) => {
        ...
        if (typeof d?.pagination?.total_items === "number")
          return d.pagination.total_items;
  ...
  useEffect(() => {
    if (targetSummary) {
      ...
      if (
        (hasSubTarget || hasProfTarget) &&
        (!hasSubTarget || isSubCompleted) &&
        (!hasProfTarget || isProfCompleted)
      ) {
        const targetId = targetSummary.id || targetSummary.userId;
        if (!localStorage.getItem(`target_achieved_${targetId}`)) {
          notify(
            `Congratulations you have achieved your ${targetSummary.targetType?.toLowerCase() || "daily"} target!`,
            "success",
          );
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      if (
        ...
        try {
          const startDt = new Date(
            `${editForm.l1_l2_date}T${editForm.l1_l2_time}:00`,
          );
          ...
          notify(
            "Status updated, but Calendar Event failed (Not connected?)",
            "error",
          );
        }
      } else {
      ...
      setTodayCandidates((prev) =>
        updateCandidateInList(prev, candidateId, editForm),
      );
  ...
  const renderRow = (
    ...
    const canSubmitTeamProfileToClient =
      showSubmitToClientBtn && isIncomingTeamSubmission;
    ...
    return (
      ...
      >
        <td style={styles.td}>
          <div>
            To: <b>{getSubmittedToName(c) || "-"}</b>
          </div>
          <div>
            By: <b style={{ color: "#27AE60" }}>{getCreatedByName(c) || ""}</b>
          </div>
        </td>
        <td style={styles.td}>
        ...
        <td style={styles.td}>
          <b>
            {truncate(c.vendor_name || c.vendor_company_name || c.vendor, 15)}
          </b>
          <br />
          <small style={styles.subStatusText}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            {showSubmitBtn &&
              (c.verification_status ? (
                ...
                <button
                  ...
                  onClick={(e) => handleOpenSubmitModal(e, c, false)}
                >
                  Submit
                </button>
              ))}
            ...
            {canSubmitTeamProfileToClient &&
              (c.client_name || c.client ? (
                ...
                <button
                  style={styles.submitBtn}
                  onClick={(e) => handleOpenSubmitModal(e, c, true)}
                >
                  Submit to Client
                </button>
              ))}
    ...
    return list.map((c, i) => {
      ...
      return (
        ...
        >
          ...
          <td style={styles.td}>{truncate(c.technology, 30)}</td>
          <td style={styles.td}>
            {c.client_name || c.client_company_name || "-"}
          </td>
          <td style={styles.td}>
            {truncate(c.requirement_title || c.jd_title || "-", 30)}
          </td>
          <td style={styles.td}>
            <span
              style={{
          ...
          <td style={styles.td}>
            <div style={{ whiteSpace: "nowrap", fontWeight: "600", color: "#2C3E50" }}>
              {c.l1_l2_date
                ? new Date(c.l1_l2_date).toLocaleDateString("en-GB", {
                    day: "numeric",
                    month: "short",
                    year: "numeric",
                  })
    ...
    <BaseLayout>
      ...
      <div style={styles.header}>
        <div>
          <h2 style={styles.welcome}>
            Welcome, {stats.user_name || "Recruiter"}
          </h2>{" "}
          {targetSummary &&
            (() => {
              const subTarget = targetSummary.totalSubmissionTarget || 0;
              ...
              return (
                <div
                  ...
                  onClick={() => navigate("/employee/my-targets")}
                >
                  <span
                    style={{
                      fontSize: "1rem",
                  ...
                  >
                    {isSubCompleted && isProfCompleted ? (
                      <span style={{ color: "#27AE60", fontWeight: "bold" }}>
                        You have completed both targets Profile Sourcing and
                        Profile Submission
                      </span>
                    ) : (
                      <>
                  ...
                  >
                    ...
                    {targetSummary.targetType?.toLowerCase() || "daily"} task
                  </span>
                </div>
              );
            })()}
        ...
        <div style={styles.btnGroup}>
          ...
          >
            <Icons.Calendar /> Connect Calendar
          </button>
          <button
            style={{
              ...styles.actionBtn,
            ...
            onClick={() => navigate("/employee/add-eod")}
          >
            <Icons.Calendar /> Add EOD
          </button>
          <button
            style={styles.actionBtn}
            onClick={() => navigate("/employee/candidates/add")}
          >
            <Icons.UserPlus /> Add Profile
          </button>
          <button
            style={styles.actionBtn}
            onClick={() => navigate("/employee/vendor/add")}
          >
            <Icons.UserPlus /> Add Vendor
          </button>
          <button
            style={styles.actionBtn}
            onClick={() => navigate("/employee/requirement/create")}
          >
            <Icons.Requirement /> Add Requirement
          </button>
        </div>
      </div>
      ...
      <div style={styles.statsGrid}>
        {[
          ...
          },
          {
            ...
            val: stats.today_submitted_profiles,
            icon: <Icons.Send />,
            col: "#FF9B51",
            url: "/employee",
          },
          {
            ...
            col: "#25343F",
            url: "/employee/requirements/my?type=today",
          },
          {
            label: "Total Vendors",
        ...
        <div
          ...
          onClick={() => navigate("/employee/my-overview")}
        >
          <div style={{ overflow: "hidden" }}>
                  ...
                  {requirementsLoading ? (
                    ...
                    activeRequirements.map((req) => (
                      <RequirementRowWrapper
                        ...
                        status={req.status}
                        onClick={() =>
                          navigate(`/employee/requirement/view/${req.id}`)
                        }
                      >
                        <td style={styles.td}>
                          <div style={styles.reqIdBadge}>
                            {req.requirement_id}
                          </div>
                        ...
                        <td style={styles.td}>
                          <span style={getRequirementStatusStyle(req.status)}>
                            {req.status || "—"}
                          </span>
                          <StatusTimer
                        ...
                        <td style={styles.td}>
                          <div
                            style={styles.infoText}
                            title={req.vendor_budget_range}
                          >
                            {truncateText(req.vendor_budget_range, 20) || "—"}
                          </div>
                        </td>
                        <td style={styles.td}>
                          <div
                        ...
                        <td style={styles.td}>
                          <div style={styles.statLine}>
                            Submissions:{" "}
                            <strong>{req.total_submissions || 0}</strong>
                          </div>
                          ...
                          <div style={styles.actionGroup}>
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/employee/requirement/view/${req.id}`,
                                );
                              }}
                            >
                              View
                            ...
                            <button
                              style={styles.editBtnReq}
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  `/employee/requirement/edit/${req.id}`,
                                );
                              }}
                            >
                              Update
      ...
      <StatusUpdateModal
        ...
        hasClientSubmission={Boolean(
          selectedCand?.client_id ||
          selectedCand?.clientId ||
          selectedCand?.client ||
          (selectedCand?.client_name && selectedCand.client_name != "N/A"),
        )}
      />
