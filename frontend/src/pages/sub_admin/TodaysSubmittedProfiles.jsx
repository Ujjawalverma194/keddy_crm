...
function TodayVerifiedProfiles() {
    ...
    const fetchTodayVerified = async (search) => {
        setLoading(true);
        try {
            let url = `/sub-admin/api/subadmin/dashboard/today-verified/`;
    ...
    const renderGroupedRows = () => {
        ...
        return candidates.map((c, i) => {
            ...
            let dateSeparator = currentDate !== lastDate ? (
                <tr key={`sep-${i}`}><td colSpan="11" style={styles.dateSeparator}>{currentDate}</td></tr>
            ) : null;
            ...
            return (
                <React.Fragment key={c.id || i}>
                    ...
                    <tr style={{ ...styles.tableRow, backgroundColor: statusStyle.bg }} onClick={() => navigate(`/sub-admin/candidate/view/${c.id}`)}>
                        ...
                        <td style={styles.td}>{truncate(c.technology || 'N/A', 25)}</td>
                        <td style={styles.td}>{c.years_of_experience_manual || '0'} Yrs</td>
                        <td style={styles.td}>{truncate(c.client_company_name || c.client_name || 'N/A', 20)}</td>
                        <td style={styles.td}>
                            <b>{truncate(c.vendor_company_name || c.vendor_name, 15)}</b><br/>
                            <small style={styles.subStatusText}>{c.vendor_number || 'N/A'}</small>
                        </td>
                        <td style={styles.td}>
                            <div>V: ₹{c.vendor_rate || '-'}</div>
                            <small style={styles.subStatusText}>C: ₹{c.client_rate || '-'}</small>
                        </td>
                        <td style={styles.td}>
  ...
  <div style={styles.btnGroup}>
            ...
            <div style={styles.header}>
                <div>
                    <h2 style={styles.welcome}>Today's Submitted Profiles ({candidates.length})</h2>
                    <p style={styles.subText}>Real-time overview of profiles submitted today.</p>
                </div>
              
            </div>

            <div style={styles.filterBar}>
                <input placeholder="Search profiles..." style={styles.searchInput} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
                ...
                <div style={styles.tableWrapper}>
                    <div style={{overflowX:'auto'}}>
                        <table style={styles.table}>
                            ...
                            <tbody>{loading ? <tr><td colSpan="11" style={styles.loadingTd}>Loading Data...</td></tr> : renderGroupedRows()}</tbody>
                        </table>
                    </div>
                </div>
            </div>
...
export default TodayVerifiedProfiles;
