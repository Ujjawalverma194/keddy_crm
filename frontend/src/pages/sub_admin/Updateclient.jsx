    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          company_name: data.company_name || data.client_company_name || "",
          number: data.phone_number || data.client_number || data.number || "",
          official_email: data.official_email || data.email || data.client_email || "",
          ...
          billing_address: data.billing_address || data.client_location || "",
...
const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  useEffect(() => {
    const searchCompany = async () => {
      if (form.company_name && !selectedCompanyId) {
        try {
          const res = await apiRequest(`/sub-admin/api/admin-clients/search/?q=${encodeURIComponent(form.company_name)}`);
          if (res && res.length > 0) {
  ...
  if (loading) return <BaseLayout><div style={{padding: '50px'}}>Loading...</div></BaseLayout>;
  return (
    <BaseLayout>
      {toast.show && (
      ...
      <div style={styles.pageShell}>
        <div style={styles.hero}>
          <div style={styles.heroLeft}>
            ...
            <div>
              <h2 style={styles.pageTitle}>Edit Client</h2>
              <p style={styles.pageSubtitle}>Update client details and POC information with documents and service details.</p>
            </div>
          </div>
                      ...
                      {showCompanySuggestions && companySuggestions.length > 0 && (
                        <ul style={{ position: 'absolute', top: '100%', left: 0, width: '150%', minWidth: '350px', background: '#fff', border: '1px solid #E5E7EB', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', listStyle: 'none', padding: 0, margin: '4px 0 0', zIndex: 10, maxHeight: '200px', overflowY: 'auto' }}>
                          {companySuggestions.map(comp => (
                            ...
                            >
                              <div>
                                  <div style={{ fontWeight: '600', color: '#111827' }}>{comp.company_name || comp.client_company_name || comp.companyName || comp.name}</div>
...
export default SubadminUpdateClient;
