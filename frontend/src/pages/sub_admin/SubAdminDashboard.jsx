...
function SubAdminDashboard() {
  ...
  const isClientSubmittedProfile = (candidate) => {
    return Boolean(
      candidate?.client ||
      candidate?.client_id ||
  ...
  const fetchSubAdminData = async () => {
    try {
      ...
      setStats(
        sData && typeof sData === "object" && !Array.isArray(sData)
          ? sData
          : {},
      );
      console.log("Dashboard Stats:", sData);
      const submittedList = toList(subData);

      setPipelineData(toList(pData));
      setSubmittedData(submittedList.filter(isClientSubmittedProfile));
...
const handleUpdateSubmit = async () => {
  try {
    ...
    const response = await apiRequest(
      `/employee-portal/api/candidates/${selectedCand.id}/update/`,
      "PUT",
      editForm
    );
  ...
  const renderRows = (list = [], showDeleteBtn = false) => {
    ...
    return rows.map((c, i) => {
      ...
      return (
        ...
        >
          ...
          <td style={styles.td}>
            <div>
              To: <b>{truncate(getSubmittedToName(c), 15) || "-"}</b>
            </div>
            <div>
              By:{" "}
              <b style={{ color: "#27AE60" }}>
                {truncate(getCreatedByName(c), 15) || "-"}
              </b>
            </div>
          </td>
          <td style={styles.td}>
            <b>{truncate(c.candidate_name || "", 15)}</b>
          </td>
          ...
          <td style={styles.td}>
            ...
            <small style={styles.subStatusText}>
              {truncate(c.client_company_name || "", 20)}
            </small>
          </td>

          <td style={styles.td}>
            <b>{truncate(c.vendor_name || c.vendor, 15)}</b>
            <br />
            <small style={styles.subStatusText}>
              {truncate(c.vendor_company_name || "", 15)}
            </small>
                ...
                {showDeleteBtn && (
                    <button
                        ...
                        onClick={async (e) => {
                            ...
                            try {
                                await apiRequest(`/sub-admin/candidates/${c.id}/soft-delete/`, 'DELETE');
      ...
      <SubAdminLayout>
        ...
        <div style={styles.header}>
          <div>
            <h2 style={styles.welcome}>
              Team Overview, {stats.user_name || "Sub Admin"}
            </h2>
        ...
        <div style={styles.statsGrid}>
          {[
            {
              label: "Team Pipeline",
              val: stats.team_pipeline,
              icon: <Icons.Pipeline />,
              col: "#4834D4",
              path: "/sub-admin/Pipeline",
            },
            {
              label: "Today's Profiles",
              val: stats.today_profiles,
              icon: <Icons.UserPlus />,
              col: "#25343F",
              path: "/sub-admin/todays-New-Profiles",
            },
            {
              label: "Today Submitted",
              val: submittedData.length || stats.today_submitted_profiles || 0,
              icon: <Icons.Send />,
              col: "#FF9B51",
              path: "/sub-admin/todays-submitted-profiles",
            },
            {
              label: "Total Submitted",
              val: stats.total_submitted_profiles,
              icon: <Icons.Send />,
              col: "#27AE60",
              path: "/sub-admin/total-submitted-profiles",
            },
            {
              label: "Total Onboarding",
              val: stats.onboard_profiles,
              icon: <Icons.Onboard />,
              col: "#27AE60",
              path: "/sub-admin/total-onbording",
            },
            {
              label: "Total Vendors",
              val: stats.total_vendors,
              icon: <Icons.Vendor />,
              col: "#25343F",
            ...
            {
              label: "Total Clients",
              val: stats.total_clients,
              icon: <Icons.Client />,
              col: "#25343F",
              path: "/sub-admin/clients",
            },
            {
              label: "Total Profiles",
              val: stats.total_profiles,
              icon: <Icons.Users />,
              col: "#25343F",
            ...
            },
            {
              ...
              icon: <Icons.Users />,
              col: "#25343F",
              path: "/sub-admin/team-manage",
            },
            {
              label: "Today's Requirements",
              val: stats.today_requirements,
              icon: <Icons.Users />,
              col: "#25343F",
              path: "/sub-admin/requirements/my?type=today",
            },
          ]?.map((s, i) => (
            <div
              key={i}
            ...
            >
              <div>
                ...
                <h3 style={{ ...styles.statValue, color: s.col }}>
                  {s.val || 0}
                </h3>
              </div>
              <div
                style={{
                  ...styles.iconCircle,
                  color: s.col,
                  backgroundColor: "rgba(37,52,63,0.05)",
                }}
              >
                {s.icon}
              </div>
          ...
          <table style={styles.table}>
            <thead style={styles.tableHeader}>
              <tr>
                ...
                <th style={styles.th}>Rate (V/C)</th>
        ...
        <Section title="Today's New Profiles">
          <table style={styles.table}>
            ...
            <tbody>{renderRows(todayProfilesData, true)}</tbody>
          </table>
        </Section>
        )}
...
// const dashboardMotionStyles = `
//   @keyframes keddyFadeUp {
//     from { opacity: 0; transform: translateY(14px); }
//     to { opacity: 1; transform: translateY(0); }
//   }
//   @keyframes keddySoftPulse {
//     0%, 100% { box-shadow: 0 18px 42px rgba(15, 23, 42, 0.06); }
//     50% { box-shadow: 0 22px 52px rgba(255, 155, 81, 0.12); }
//   }
//   .keddy-dashboard-shell { animation: keddyFadeUp 0.45s ease both; }
//   .keddy-action-btn, .keddy-stat-card, .keddy-section-card, .keddy-table-row, .keddy-edit-btn {
//     transition: transform 0.22s ease, box-shadow 0.22s ease, border-color 0.22s ease, background 0.22s ease, opacity 0.22s ease;
//   }
//   .keddy-action-btn:hover {
//     transform: translateY(-2px);
//     box-shadow: 0 14px 28px rgba(255, 155, 81, 0.26) !important;
//     filter: saturate(1.04);
//   }
//   .keddy-action-btn:active { transform: translateY(0px) scale(0.99); }
//   .keddy-stat-card:hover {
//     transform: translateY(-5px);
//     box-shadow: 0 22px 46px rgba(15, 23, 42, 0.12) !important;
//     border-color: rgba(255, 155, 81, 0.45) !important;
//   }
//   .keddy-stat-card:hover .keddy-stat-icon {
//     transform: rotate(-4deg) scale(1.08);
//     box-shadow: 0 12px 26px rgba(255, 155, 81, 0.18);
//   }
//   .keddy-section-card:hover {
//     box-shadow: 0 20px 48px rgba(15, 23, 42, 0.09) !important;
//     border-color: rgba(226, 232, 240, 1) !important;
//   }
//   .keddy-table-row:hover {
//     transform: translateY(-1px);
//     filter: brightness(0.995);
//     box-shadow: inset 4px 0 0 #FF9B51, 0 8px 18px rgba(15, 23, 42, 0.05);
//   }
//   .keddy-edit-btn:hover {
//     transform: translateY(-2px) scale(1.04);
//     background: #25343F !important;
//     color: #fff !important;
//     box-shadow: 0 10px 20px rgba(37, 52, 63, 0.18);
//   }
//   .keddy-edit-btn:active { transform: scale(0.98); }
// `;
...
//     <>
//       <SubAdminLayout>
//         <style>{dashboardMotionStyles}</style>
//         <div style={styles.dashboardShell} className="keddy-dashboard-shell">
//         {toast.show && (
//           <div
//             style={{
//               ...styles.toast,
...
// const Section = ({ title, children }) => (
//   <div style={styles.sectionContainer} className="keddy-section-card">
//     <div style={styles.sectionHeader}>
//       <h3 style={styles.sectionTitle}>{title}</h3>
//     </div>
//     <div style={styles.tableWrapper}>
...
//   },
//   actionBtn: {
//     background: "linear-gradient(135deg, #FFB36F 0%, #FF7F37 55%, #FF5E2F 100%)",
//     color: "#fff",
//     border: "none",
//     padding: "10px 16px",
//     borderRadius: "14px",
...
//     fontSize: "13px",
//     minHeight: "42px",
//     boxShadow: "0 10px 22px rgba(255, 126, 55, 0.20)",
//     whiteSpace: "nowrap",
//   },
//   statsGrid: {
//     display: "grid",
...
//   },
//   badge: {
//     background: "rgba(255, 155, 81, 0.14)",
//     color: "#FF7F37",
//     padding: "5px 10px",
//     borderRadius: "999px",
//     fontSize: "11px",
