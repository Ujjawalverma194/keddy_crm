...
function ClientList() {
    ...
    const getApiProvidedProfileCount = (client) =>
        toSafeNumber(
            client?.submitted_profiles_count ??
            client?.total_submitted_profiles ??
            client?.submitted_profile_count ??
            client?.profiles_count ??
            client?.profile_count ??
            client?.candidate_count ??
            client?.candidates_count ??
            0
        );

    const getCandidateClientId = (candidate) =>
        candidate?.client_id ??
        candidate?.clientId ??
        candidate?.client?.id ??
        candidate?.Client?.id ??
        candidate?.client_details?.id ??
        candidate?.clientDetails?.id ??
        candidate?.submitted_to_client_id ??
        candidate?.submitted_client_id ??
        candidate?.submitted_to_id ??
        candidate?.client;

    const getCandidateClientNames = (candidate) => [
        candidate?.client_name,
        candidate?.client_company_name,
        candidate?.company_name,
        candidate?.submitted_to_name,
        candidate?.submitted_client_name,
        candidate?.client?.client_name,
        candidate?.client?.company_name,
        candidate?.Client?.client_name,
        candidate?.Client?.company_name,
        candidate?.client_details?.client_name,
        candidate?.client_details?.company_name,
        candidate?.clientDetails?.client_name,
        candidate?.clientDetails?.company_name,
    ].filter(Boolean).map(normalizeText);

    const doesCandidateBelongToClient = (candidate, client) => {
        const candidateClientId = getCandidateClientId(candidate);

        if (
            candidateClientId !== null &&
            candidateClientId !== undefined &&
            String(candidateClientId) === String(client.id)
        ) {
            return true;
        }

        const candidateClientNames = getCandidateClientNames(candidate);
        const clientNames = [
            client?.client_name,
            client?.company_name,
        ].filter(Boolean).map(normalizeText);

        return clientNames.some((name) => name && candidateClientNames.includes(name));
    };
        ...
        try {
            ...
            clientList.forEach((client) => {
                const calculatedCount = allProfiles.filter((candidate) =>
                    doesCandidateBelongToClient(candidate, client)
                ).length;

                counts[client.id] = calculatedCount || getApiProvidedProfileCount(client);
            });
            ...
            clientList.forEach((client) => {
                fallbackCounts[client.id] = getApiProvidedProfileCount(client);
            });
            setClientProfileCounts(fallbackCounts);
        } finally {
            setProfileCountsLoading(false);
        }
    };

    const getClientSubmittedProfileCount = (client) => {
        const calculatedCount = clientProfileCounts?.[client.id];

        if (calculatedCount !== null && calculatedCount !== undefined && calculatedCount !== "") {
            return toSafeNumber(calculatedCount);
        }

        return getApiProvidedProfileCount(client);
    };

    const renderProfileCount = (client) => {
        if (profileCountsLoading && clientProfileCounts?.[client.id] === undefined) {
            return "Loading...";
        }

        return `${getClientSubmittedProfileCount(client)} Profiles`;
    };
    ...
    const fetchClients = async (page = 1, search = "") => {
        ...
        try {
            ...
            const clientResults = response.results || [];
            setClients(clientResults);
            fetchClientSubmittedProfileCounts(clientResults);
